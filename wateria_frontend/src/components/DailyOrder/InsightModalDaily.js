...
import { getDailyOrderStats } from "../../api/DailyOrderApi";

export default function InsightModalDaily({ show, onHide }) {
 ...
 const getDateRange = () => {
  ...
  const fetchInsights = async () => {
    ...
    setLoading(true);
    const data = await getDailyOrderStats(startDate, endDate);
    setInsights(data);
    setLoading(false);
  };
  ...
  return (
    <Modal show={show} onHide={onHide} size="md">
      <Modal.Header closeButton>
        <Modal.Title>Daily Order Insights</Modal.Title>
      </Modal.Header>
      <Modal.Body>
